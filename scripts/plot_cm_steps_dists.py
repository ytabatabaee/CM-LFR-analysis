...
def vertical_line_10(**kwargs):
    ...
    plt.axvline(10, linestyle = '--', color='grey', ls='--', lw=1.2)
...
if __name__ == "__main__":
    ...
    for net in net_list:
        ...
        for r in resolutions:
            ...
            df = pd.read_csv('S4_'+net+'_leiden'+r_str+'_i2_make_cm_ready.R_csizes.tsv')
            df['res'] = r
            df['type'] = 'Empirical'
            df['name'] = net
            df['step'] = 'Filter'
            ...
            df = pd.read_csv('S6_' + net + '_leiden' + r_str + '_i2_post_cm_filter.R_csizes.tsv')
            df['res'] = r
            df['type'] = 'Empirical'
            df['name'] = net
            df['step'] = 'post-CM'
            ...
            try:
                ...
                df = pd.read_csv('S4_' + net + '_lfr_leiden' + r_str + '_i2_make_cm_ready.R_csizes.tsv')
                df['res'] = r
                df['type'] = 'LFR'
                df['name'] = net
                df['step'] = 'Filter'
                ...
                df = pd.read_csv('S6_' + net + '_lfr_leiden' + r_str + '_i2_post_cm_filter.R_csizes.tsv')
                df['res'] = r
                df['type'] = 'LFR'
                df['name'] = net
                df['step'] = 'post-CM'
    ...
    all_dfs.to_csv('all_cm_steps_dists.csv')

    
    '''g = sns.FacetGrid(all_dfs[(all_dfs['name']=='open_citations')], col="step", row="res", margin_titles=True, height=4,
                      row_order=['0.5', '0.1', '0.01', '0.001', '0.0001', 'modularity'])
    g.map_dataframe(sns.scatterplot, x="x", y="count", hue='type', palette='Dark2', alpha=0.55, linewidth=0).set(yscale = 'log', xscale='log')
    ...
    g.fig.text(0.35, 0.13, s='Cluster size', fontsize=20)
    g.fig.text(0.08, 0.4, s='Cluster count', rotation=90, fontsize=20)
    #g.add_legend(fontsize=16)
    g.add_legend(bbox_to_anchor=(0.4, 0.87), fontsize=20, loc='upper center', ncol=2, frameon=True)
    #g.add_legend(bbox_to_anchor=(0.45, 0.87), fontsize=20, loc='upper center', ncol=2, frameon=True)
    #for ax in g.axes[0]:
        #ax.vlines(x=10, ymin=0, ymax=10*7, colors='grey', ls='--', lw=0.5)'''
    ...
    #plt.savefig('oc_steps.pdf', bbox_inches='tight')
